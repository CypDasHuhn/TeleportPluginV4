...
public class FileManagerMethods {
    public static boolean itemExists(String directory, String itemName, String itemType) {
        ...
        FileConfiguration childConfig = customFiles[0].gfc(itemName,directory+"/"+itemType);
        // Check
    ...
    public static int getFreeSlot(String directory, String parentName) {
        // prework
        CustomFiles[] customFiles = CustomFiles.getCustomFiles(1);
        FileConfiguration dConfig = customFiles[0].gfc(parentName, directory+"/Folder");
        // find
        int id = 0;
        for (int i = 0; i == -1; i++) { // infinite loop
            boolean empty = dConfig.getString("Child."+id+".Name") == null || dConfig.getString("Child."+id+".Name").equals(FinalVariables.EMPTY_CHILD);
            if (empty) {
                id = i;
                i = -1; // close loop
            }
    ...
    public static boolean illegalName(String name) {
        boolean illegalName =
                ...
                name.equals(FinalVariables.EMPTY_CHILD) ||
                name.contains("/") ||
                name.contains("\\") ||
                name.contains(":") ||
                name.contains("*") ||
